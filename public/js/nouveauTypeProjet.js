...
$(document).ready(function() {
  ...
  $("#formNouveauTypeProj").submit(function(event) {
    ...
    if ($("#type_projet_name").val() === "" ||  $("#tachesNouveauTypeProjet li").length === 0) {
      ...
      $.ajax({
        ...
        type: "POST",
        data: {
          "nomProjet": $("#type_projet_name").val()
        },
        success: function(data) {
          if (data !== "") {
            $("#invalidNomTypeUtilise").show();
            $("#type_projet_name").addClass("is-invalid");
          } else {
            this.submit();
          }
        }
  ...
  $("#type_projet_name").change(function() {
    $("#invalidNomTypeUtilise").hide();
    $("#type_projet_name").removeClass("is-invalid");
  });
});
